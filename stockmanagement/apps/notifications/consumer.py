...
class NotificationConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        query_string = (
            parse_qs(self.scope["query_string"].decode()) if self.scope["query_string"] else {}
        )
        ...
        if token:
            ...
            logger.info(
                f"Tentative d'authentification par token,"
        ...
        await self.send(
            text_data=json.dumps({"type": "initial_notifications", "notifications": notifications})
        )
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            if command == "mark_read":
                ...
                await self.send(
                    text_data=json.dumps(
                        {"type": "error", "message": f"Commande inconnue: {command}"}
                    )
                )
            ...
            await self.send(
                text_data=json.dumps({"type": "error", "message": "Format de message invalide"})
            )
        except Exception as e:
            logger.error(f"📥 Erreur lors du traitement du message: {str(e)}")
            await self.send(
                text_data=json.dumps({"type": "error", "message": f"Erreur serveur: {str(e)}"})
            )
    ...
    async def send_notification(self, event):
        ...
        await self.send(text_data=json.dumps({"type": "new_notification", **event["data"]}))
    ...
    def get_unread_notifications(self):
        ...
        notifications = NotificationService.get_user_notifications(
            user_id=self.user.id, status="UNREAD", limit=10  # Limiter à 10 dernières notifications
        )
    ...
    def get_user_notifications(self, status=None, limit=None):
        ...
        notifications = NotificationService.get_user_notifications(
            user_id=self.user.id, status=status, limit=limit
        )
