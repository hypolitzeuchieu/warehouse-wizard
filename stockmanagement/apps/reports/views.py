...
from apps.reports.models import Expense, InvoiceArchive, Report, Treasure
...
class InvoiceViewSet(viewsets.ViewSet):
    ...
    def create_invoice(self, request):
        ...
        if serializer.is_valid():
            try:
                ...
                return Response({f"error: {str(e)}"}, status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            try:
                ...
                if pdf_file.success:
                    ...
                    response["Content-Disposition"] = (
                        f"attachment; " f"filename=invoice_{invoice_id}.pdf"
                    )
                    return response

                return Response({"error": "Invoice not found"}, status.HTTP_404_NOT_FOUND)
    ...
    @swagger_auto_schema(
        ...
        responses={200: InvoiceSerializer, 400: "Bad Request", 500: "Internal Server Error"},
    )
    ...
    def get_invoices(self, request):
        ...
        if query_serializer.is_valid():
            ...
            try:
                invoices = service.get_invoices(start_date=start_date, end_date=end_date)
                if not invoices.success:
                    return Response({"error": invoices.error}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if serializer.is_valid():
            try:
                ...
                result = service.update_invoice(
                    invoice_id=invoice_id, updated_data=updated_data, user=user
                )
...
class GeneralReportViewSet(viewsets.ViewSet):
    ...
    def get_general_report(self, request):
        ...
        if query_serializer.is_valid():
            try:
                ...
                response_data = {"success": report_result.success}
                ...
                return Response(
                    response_data,
                    status=(
                        status.HTTP_200_OK if report_result.success else status.HTTP_400_BAD_REQUEST
                    ),
                )
        ...
        logger.error(f"Invalid data provided: {query_serializer.errors}")
        return Response(
            {"success": False, "error": query_serializer.errors}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def get_all_reports(self, request):
        try:
            reports = Report.objects.select_related("inventory_report", "sales_report").order_by(
                "-generated_at"
            )
    ...
    def create_expense(self, request):
        ...
        if serializer.is_valid():
            try:
                ...
                result = ExpenseService.create_expense(amount, expense_type, reason, created_by)

                if result.success:
                    return Response(
                        ExpenseSerializer(result.data).data, status=status.HTTP_201_CREATED
                    )
    ...
    def update_expense(self, request, pk=None):
        ...
        try:
            ...
            if expense.success:
                ...
                return Response(ExpenseSerializer(expense.data).data, status=status.HTTP_200_OK)
        ...
        if query_serializer.is_valid():
            ...
            try:
                ...
                if not expenses_summary.success:
                    return Response(
                        {"error": expenses_summary.error}, status=status.HTTP_400_BAD_REQUEST
                    )
                ...
                response_data = {"total_amount": total_amount, "categories": paginated_categories}
