...
class Invoice(models.Model):
    ...
    refund_amount = models.DecimalField(max_digits=10, decimal_places=3, default=0.00)
    advance_paid = models.DecimalField(max_digits=15, decimal_places=3, default=0.00)
    _remaining_amount = models.DecimalField(max_digits=10, decimal_places=3, default=0.00)
...
class InvoiceLine(models.Model):
    ...
    discount = models.DecimalField(max_digits=10, decimal_places=3, default=0.00)
...
class InventoryReport(models.Model):
    ...
    report = models.OneToOneField(
        Report, on_delete=models.CASCADE, related_name="inventory_report", null=True
    )
...
class SalesReport(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    report = models.OneToOneField(
        Report, on_delete=models.CASCADE, related_name="sales_report", null=True
    )
...
class InvoiceArchive(models.Model):
    ...
    tax = models.DecimalField(max_digits=5, decimal_places=2, default=0.00)
    reason = models.TextField(blank=True, null=True)
    refund_amount = models.DecimalField(max_digits=15, decimal_places=3, default=0.00)
    advance_paid = models.DecimalField(max_digits=15, decimal_places=3, default=0.00)
    _remaining_amount = models.DecimalField(max_digits=15, decimal_places=3, default=0.00)
    due_date = models.DateField(blank=True, null=True)
    is_credit_settled = models.BooleanField(default=False)
...
class InvoiceArchiveLine(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    invoice = models.ForeignKey(InvoiceArchive, on_delete=models.CASCADE, related_name="lines")
...
class Expense(models.Model):
    ...
    updated_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="updated_expenses"
    )
