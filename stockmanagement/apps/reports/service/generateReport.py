...
from apps.reports.models import Expense, InventoryReport, Invoice, InvoiceLine, Report, SalesReport
from apps.reports.service.entities import ServiceResponse
from apps.stock.models import Product, Stock
...
class GenerateReportService:
    ...
    def decimal_to_json_serializable(obj):
        ...
        if isinstance(obj, decimal.Decimal):
            ...
            return {
                k: GenerateReportService.decimal_to_json_serializable(v) for k, v in obj.items()
            }
    ...
    def generate_report(
        ...
        try:
            ...
            report_data = GenerateReportService._get_report_data(report_type, start_date, end_date)
            ...
            with transaction.atomic():
                ...
                if report_type == "inventory":
                    ...
                    inventory_report.stocks.set(
                        Stock.objects.filter(created_at__date__range=[start_date, end_date])
                    )
    ...
    @staticmethod
    def _get_report_data(report_type: str, start_date=None, end_date=None):
        if report_type == "inventory":
            return GenerateReportService._generate_inventory_data(start_date, end_date)
        elif report_type == "sales":
            sales_data_response = GenerateReportService._generate_sales_data(start_date, end_date)
    ...
    def _generate_inventory_data(start_date=None, end_date=None):
        ...
        near_expiry = products.filter(expiry_date__range=(now, soon), is_expired=False)
    ...
    @staticmethod
    def _generate_sales_data(start_date=None, end_date=None):
        try:
            ...
            credit_invoices = Invoice.objects.filter(
                status="CREDIT", created_at__date__range=[start_date, end_date]
            )
            ...
            completed_revenue = (
                InvoiceLine.objects.filter(invoice__in=completed_invoices).aggregate(
            ...
            total_credit_revenue = (
                InvoiceLine.objects.filter(invoice__in=credit_invoices).aggregate(
                    total=Sum("line_total")
                )["total"]
                or 0
            )

            total_advance_paid = credit_invoices.aggregate(total=Sum("advance_paid"))["total"] or 0
            total_remaining_amount = (
                credit_invoices.aggregate(total=Sum("_remaining_amount"))["total"] or 0
            )
            ...
            discount = (
                InvoiceLine.objects.filter(invoice__in=invoice_ids).aggregate(
            ...
            total_purchase = InvoiceLine.objects.filter(invoice__in=invoice_ids).aggregate(
            ...
            expense_details = [
                ...
                for e in Expense.objects.filter(created_at__date__range=(start_date, end_date))
            ]
    ...
    def download_report(report_id):
        try:
            ...
            html_string = render_to_string(
                f"{report_type}_report_template.html", {"data": specific.data}
            )
