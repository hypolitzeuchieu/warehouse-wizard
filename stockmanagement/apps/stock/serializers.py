...
def validate_product_id_or_barcode(attrs):
    ...
    if product_id and barcode:
        raise serializers.ValidationError("Only one of product_id or barcode should be provided.")
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    purchase_price = serializers.DecimalField(max_digits=10, decimal_places=2, write_only=True)
    ...
    def validate(self, attrs):
        ...
        if not attrs.get("product_id") and not attrs.get("barcode"):
            ...
            if promo_price is None:
                raise serializers.ValidationError(
                    {"promo_price": "Promo price is required " "when the product is on promotion."}
                )
