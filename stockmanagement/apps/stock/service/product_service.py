...
class ProductService:
    ...
    def create_product(
        ...
        try:
            ...
            barcode_image_url = BarcodeService.create_and_upload_barcode_image(
                barcode_value, filename=f"{name}_{uuid.uuid4().hex[:8]}"
            )
            ...
            with transaction.atomic():
                ...
                (
                    logger.info(
                        f"Product successfully created: {product.name}" f" with ID: {product.id}"
                    )
                    if created
                    else None
                )
            ...
            return StockServiceResponse(
                False, error=f"Category with ID {category_id} does not exist."
            )
        except Exception as e:
            return StockServiceResponse(False, error=f"An unexpected error occurred: {str(e)}")
    ...
    def update_product(product_id, data, updated_by) -> StockServiceResponse:
        try:
            ...
            return StockServiceResponse(False, error=f"Product with ID {product_id} not found.")
        except Exception as e:
            return StockServiceResponse(False, error=f"An unexpected error occurred: {str(e)}")
    ...
    def get_all_stock() -> StockServiceResponse:
        ...
        """
        try:
            stocks = Stock.objects.select_related("product", "category", "subcategory").all()
    ...
    def get_products_by_expiry_date() -> StockServiceResponse:
        ...
        try:
            ...
            expired_products = Product.objects.filter(expiry_date__lt=now).select_related(
    ...
    @staticmethod
    def add_product_quantity(product_id: str, quantity: int, updated_by) -> StockServiceResponse:
        """
    ...
    @staticmethod
    def reduce_product_quantity(product_id: str, quantity: int, updated_by) -> StockServiceResponse:
        """
