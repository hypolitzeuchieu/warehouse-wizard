...
from apps.authentication.permissions import IsManagerPermission, IsStorekeeper
from apps.stock.models import Category, StockMovement, SubCategory
from apps.stock.serializers import (
    CategorySerializer,
    GetProductCategorySerializer,
    GetProductSubCategorySerializer,
    ...
    def process_stock_movement(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                if result.success:
                    return Response(
                        StockMovementSerializer(result.data).data, status=status.HTTP_201_CREATED
                    )
                return Response({"error": result.error}, status=status.HTTP_400_BAD_REQUEST)
            except Exception as e:
                logger.error(f"Unexpected error occurred: {str(e)}")
    ...
    def get_stock_movements(self, request):
        ...
        try:
            ...
            page = paginator.paginate_queryset(
                queryset=StockMovement.objects.all(), request=request, view=self
            )
...
class CategoryViewSet(viewsets.ViewSet):
    ...
    def get_categories(self, request):
        ...
        try:
            ...
            page = paginator.paginate_queryset(
                queryset=Category.objects.all(), request=request, view=self
            )
    ...
    def delete_subcategory(self, request, pk=None):
        ...
        try:
            ...
            subcategory.delete()
            return Response(
                {"message": "Subcategory deleted successfully"}, status=status.HTTP_204_NO_CONTENT
            )
        except SubCategory.DoesNotExist:
            return Response({"error": "Subcategory not found"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def get_subcategories(self, request):
        ...
        try:
            ...
            page = paginator.paginate_queryset(SubCategory.objects.all(), request, view=self)
    ...
    def get_products_by_subcategory(self, request):
        ...
        if serializer.is_valid():
            ...
            page_size = serializer.validated_data.get("page_size", 10)
            try:
                products = StockService().get_products_by_subcategory(subcategory_id)
                if products.success:
                    paginator = CustomPagination()
                    paginator.page_size = page_size
                    result_page = paginator.paginate_queryset(products.data, request)
...
class ProductViewSet(viewsets.ViewSet):
    ...
    def update_product(self, request):
        ...
        if serializer.is_valid():
            ...
            if response.success:
                return Response(ProductSerializer(response.data).data, status=status.HTTP_200_OK)
            return Response({"error": response.error}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def get_products_by_expiry_date(self, request):
        ...
        try:
            ...
            if result.success:
                serializer = ProductSerializer(result.data["expired_products"], many=True)
                near_expiry_product = ProductSerializer(result.data["near_expiry"], many=True)
                return Response(
                    {
