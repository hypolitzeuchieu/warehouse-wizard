...
from apps.reports.models import Expense, Invoice, InvoiceLine
from apps.reports.service.invoice_service import ServiceResponse
...
class DashboardService:
    ...
    @staticmethod
    def calculate_trend(current: float, previous: float) -> dict[str, str | float]:
        if previous == 0:
            if current == 0:
    ...
    def get_dashboard_stats(start_date, end_date) -> ServiceResponse:
        try:
            ...
            expenses = Expense.objects.filter(created_at__date__range=[previous_start, end_date])
            DashboardService._process_expenses(
                expenses, start_date, end_date, previous_start, daily_data, previous_daily_data
            )

            for invoice in invoices:
                DashboardService._process_invoice(
                    invoice, start_date, end_date, previous_start, daily_data, previous_daily_data
                )
    ...
    def _calculate_invoice_profit(invoice):
        ...
        for line in invoice.lines.all():
            if (
                hasattr(line, "product")
                and line.product
                and hasattr(line.product, "purchase_price")
            ):
    ...
    @staticmethod
    def _generate_daily_results(start_date, end_date, daily_data, previous_daily_data):
        results = []
        current_day = start_date
        while current_day <= end_date:
            current_data = daily_data.get(current_day, DashboardService._initialize_daily_data())
    ...
    def get_top_selling_products(start_date, end_date) -> ServiceResponse:
        ...
        try:
            ...
            # Top products query
            top_products = (
                lines.values("product__id", "product__name")
                .annotate(
                    sold=Sum("quantity"),
                    revenue=Sum("line_total"),
                    profit=Sum(
                ...
                "top_products": [
                    {
                        ...
                        "margin": (
                            round((float(p["profit"]) / float(p["revenue"])) * 100, 1)
                            if p["revenue"] and float(p["revenue"]) > 0 and p["profit"]
                            else 0
                        ),
                    }
                ...
                "top_categories": [
                    {
                        ...
                        "margin": (
                            round((float(c["profit"]) / float(c["revenue"])) * 100, 1)
                            if c["revenue"] and float(c["revenue"]) > 0 and c["profit"]
                            else 0
                        ),
                    }
        ...
        try:
                ...
                stock_data.append(
                    {
                        ...
                        "status_text": (
                            ...
                            else "Stock faible" if product.quantity > 0 else "Rupture de stock"
                        ),
                    }
                )
            ...
            alerts = {
                ...
                "critical": Product.objects.filter(
                    Q(quantity=0) | Q(quantity__lt=F("min_quantity") * 0.5, quantity__gt=0)
                ).count(),
            }

            data = {"stockStatus": stock_status, "stockData": stock_data, "alerts": alerts}
        ...
        try:
            ...
            recent_invoices = (
                ...
                .annotate(
                    ...
                    profit=Sum(
                        ExpressionWrapper(
                            (F("lines__unit_price") - F("lines__product__purchase_price"))
                            * F("lines__quantity"),
                            output_field=DecimalField(),
                        )
                    ),
                ...
                result.append(
                    {
                        ...
                        "margin": (
                            round((float(invoice.profit) / float(invoice.invoice_total)) * 100, 1)
