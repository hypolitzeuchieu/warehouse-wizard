...
class ReportService:
    ...
    def generate_inventory_report(
        ...
        try:
            ...
            stocks_in_range = Stock.objects.filter(created_at__range=[start_date, end_date])
            report.stocks.set(stocks_in_range)
    ...
    def get_inventory_data(start_date=None, end_date=None) -> ServiceResponse:
        ...
        try:
            ...
            report = InventoryReport.objects.filter(
...
class GenerateReportService:
    ...
    @staticmethod
    def generate_report(report_type: str, user) -> ServiceResponse:
        """
        Generate a report dynamically based on type and save it to the database.
        """
        try:
            if report_type not in dict(Report.REPORT_TYPE_CHOICES):
                return ServiceResponse(success=False, error='Invalid report type.')

            report_data = GenerateReportService._get_report_data(report_type)
            ...
            return ServiceResponse(
                success=True, data={'report_id': report.id, 'report_data': report_data.data}
            )
    ...
    def _get_report_data(report_type: str):
        """
        Retrieve data based on the report type.
        """
        if report_type == 'inventory':
            return GenerateReportService._generate_inventory_data()
        elif report_type == 'sales':
            return GenerateReportService._generate_sales_data()
        # elif report_type == "returns":
        #     return GenerateReportService._generate_returns_data()
        elif report_type == 'expired':
            return GenerateReportService._generate_expired_products_data()
        return {}
    ...
    # @staticmethod
    # def _generate_returns_data():
    #     """
    #     Generate data for returns report.
    #     """
    #     returns = Return.objects.all()
    #     return {
    #         "total_returns": returns.count(),
    #         "total_lost_revenue": returns.aggregate(total=F("amount"))["total"] or 0,
    #     }
    @staticmethod
    def _generate_expired_products_data():
        """
        Generate data for expired products report.
        """
